 ...
 * всех точках). Поставщик берётся из самой закупки/строки автоматически —
 * пользователь его никогда не вводит вручную, так что в интерфейсе это
 * никак не усложняет ввод названия. Но в ключе он нужен: на реальных
 * данных ~60% названий товаров в матрице текстово совпадают сразу у
 * НЕСКОЛЬКИХ разных поставщиков ("Сливки 33%" и т.п.) — без поставщика в
 * ключе привязка для одного поставщика тихо "перетекала" бы и на чужие
 * закупки с тем же текстом у совсем другой компании.
 *
 * targetProduct — третий сегмент ключа matching.planPairs/planPairsByPack
 * (норм.), то есть "вот эта строка матрицы". supplier/rawProduct — их же
 * написание, только для показа в Справочниках, на сам лукап не влияют.
 */
