...
function resolveRowPlan(b: BaseRow, edits: Edits, matching: MatchingTable, designatedIndex: DesignatedIndex, knownFlatPairs: Set<string>): Resolved {
  ...
  // Текст фасовки у факта и у матрицы может не совпасть буквально по куче
  // причин, которые не про разный товар: iiko иногда пишет голую единицу
  // ("л", "кг") вместо полной фасовки с брендом ("«чудское озеро» 1л"), для
  // весовых товаров (сыр, колбаса) конкретный вес куска каждый раз свой, или
  // поставщик привёз тот же товар под другим брендом/тарой (напр. "Соус
  // ореховый" от ФудМаркет обычно "Тамаки" 1,5л, но разово приехал "Махеев"
  // 770гр — тот же по сути товар, просто другая партия). Раз у поставщика
  // тут ровно ОДИН вариант фасовки — гадать, тот ли это товар, не нужно, это
  // и есть доказательство: другому просто неоткуда взяться. Цену сознательно
  // не проверяем (раньше проверяли — см. Ayakaz-баг в истории; но у Рене
  // расхождение цены тут само по себе нормальная ситуация — почти вдвое
  // дороже "Тамаки" за "Махеев" это ожидаемо, не признак ошибки), только
  // одно совпадение "одна известная фасовка у этого поставщика" — этого
  // достаточно.
  if (pack) {
