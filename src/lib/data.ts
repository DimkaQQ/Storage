 ...
 * убранного автоподбора "единственная прайсованная фасовка = она". Сюда
 * НЕ включены:
 ...
 *    перепутало голубику с малиной у Azik Trade);
 *  - Соус соевый/Коженков ("амой" — название бренда, не факт, что тот же
 *    объём), Соус майонез/Royal Kitchen (500гр против 5л/4.7кг — слишком
 *    большая разница, чтобы быть уверенным), Тунец конс./Azik Trade (в
 *    матрице сама фасовка "180гр"/"170гр" гуляет от месяца к месяцу — нет
 *    единственного устойчивого варианта), Лепешка Роти/Euro Food KZ и
 *    Краситель пищевой гелевый/Коженков — тут одна прайсованная фасовка,
 *    но по этим же товарам уже ловили похожую путаницу (Roti на разные
 *    фасовки цена разная; краситель обычно бывает разных цветов) — эти
 *    остались "нет в матрице" с подсказкой, поправить вручную, если
 *    сверили и уверены.
 */
...
function resolveRowPlan(b: BaseRow, edits: Edits, matching: MatchingTable, designatedIndex: DesignatedIndex, knownFlatPairs: Set<string>): Resolved {
  ...
  // просто без клика.
  const curatedTarget = !packAlias && packFixKey ? CURATED_PACK_FIXES[packFixKey] : undefined
