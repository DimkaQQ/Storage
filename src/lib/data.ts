 ...
 * сопоставление от этого не страдают (ключ точный), но описание — нет,
 * поэтому оно и скрывается safeLabel'ом ниже; здесь просто объясняем, почему. */
function labelConflictNote(product: string, label: string | null | undefined): string | null {
  ...
  if (pf && lf && pf !== lf) return `В матрице описание товара противоречит закупке: у них «${FREEZE_RU[lf]}», по факту «${FREEZE_RU[pf]}» — название скрыто, стоит поправить в «Сырьё F».`
  const pb = boneState(product), lb = boneState(label)
  if (pb && lb && pb !== lb) return `В матрице описание товара противоречит закупке: у них «${BONE_RU[lb]}», по факту «${BONE_RU[pb]}» — название скрыто, стоит поправить в «Сырьё F».`
  return null
}
...
export interface Row {
  ...
  labelConflict: boolean  // productLabel спрятан из-за нестыковки внутри самой матрицы (её ключ vs её же описание) — подсветить строку, объяснение уже в note
...
export function computeRows(base: BaseRow[], edits: Edits, matching: MatchingTable = BUNDLED_MATCHING): Row[] {
  ...
  const rows: Row[] = base.map((b) => {
    ...
    // labelConflict — расхождение в самой матрице (её ключ товара
    // противоречит её же текстовому описанию) — показываем всегда, даже
    // если у закупки уже есть свой комментарий из iiko: это разные вещи
    // (одно — их слова про эту поставку, другое — нестыковка в «Сырьё F»),
    // молчать о втором ради первого не стоит.
    const parts: string[] = []
