...
export default function PurchaseDetail() {
  ...
  const { purchases, suppliers, venues, inventory, updatePurchaseStatus, deletePurchase } = useStore()

  const purchase = purchases.find((p) => p.id === id)
  const supplier = suppliers.find((s) => s.id === purchase?.supplierId)
  const venue = venues.find((v) => v.id === purchase?.venueId)
    ...
    <div className="p-4 lg:p-6 space-y-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-xl num" style={{ color: 'var(--white)' }}>Заказ #{purchase.id.slice(-4).toUpperCase()}</h1>
          <p className="text-sm num" style={{ color: 'var(--muted)' }}>{purchase.createdAt.slice(0, 10)}</p>
        </div>
      ...
      <div className="card" style={{ borderColor: 'rgba(200,168,75,0.2)' }}>
        <div className="flex items-start justify-between mb-2">
          <p className="text-sm" style={{ color: 'var(--muted)' }}>{supplier?.name}</p>
          {venue && <span className="badge badge-ordered">{venue.name}</span>}
        </div>
        <p className="text-3xl font-bold num" style={{ color: 'var(--gold)' }}>
          {formatPrice(purchase.totalAmount)}
        </p>
        <div className="grid grid-cols-2 gap-3 mt-4">
          <div>
            <p className="label">Создан</p>
            <p className="text-sm font-medium num" style={{ color: 'var(--white)' }}>{purchase.createdAt.slice(0, 10)}</p>
          </div>
          <div>
            <p className="label">Ожидается</p>
            <p className="text-sm font-medium num" style={{ color: 'var(--white)' }}>{purchase.expectedDate?.slice(0, 10) ?? '—'}</p>
          </div>
          {purchase.receivedDate && (
            <div>
              <p className="label">Получен</p>
              <p className="text-sm font-medium num" style={{ color: 'var(--green)' }}>{purchase.receivedDate.slice(0, 10)}</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="card p-0">
          ...
          {purchase.items.map((item, idx) => {
            ...
            return (
              <div
                ...
                style={{ borderBottom: idx < purchase.items.length - 1 ? '1px solid rgba(255,255,255,0.03)' : 'none' }}
        ...
        <div className="flex justify-between items-center px-5 py-4" style={{ borderTop: '1px solid var(--border)' }}>
          ...
          <span className="text-lg font-bold num" style={{ color: 'var(--gold)' }}>
            {formatPrice(purchase.totalAmount)}
          </span>
        </div>
      </div>
      ...
      {(purchase.status === 'pending' || purchase.status === 'cancelled') && (
        <button
          ...
          style={{ color: 'var(--red)', border: '1px solid rgba(255,69,58,0.2)', background: 'transparent', cursor: 'pointer' }}
